document.addEventListener('DOMContentLoaded', function() {
    ...
    let totalTargets = 25;
    let completedLevels = [];
    let timeLeft = 30; // Add timer variable
    ...
    function startClickLevel() {
        gameDisplay.innerHTML = `
            <div class="click-instruction">
                Clique nos círculos vermelhos!<br>
                <strong>Meta: 25 pontos em 30 segundos</strong>
            </div>
        `;
        createTarget();
    }
    ...
    function startScrollLevel() {
        ...
        instruction.textContent = 'Role a página para encontrar e clicar nos alvos laranja!';
        scrollContent.appendChild(instruction);
        
        // Add some content to make it scrollable
        for (let i = 0; i < 20; i++) {
            const filler = document.createElement('div');
            filler.textContent = i % 3 === 0 ? 'Role para baixo para encontrar os alvos' : '...';
            filler.style.padding = '20px 0';
            scrollContent.appendChild(filler);
            
            // Add targets at specific positions
            if (i === 5 || i === 10 || i === 15 || i === 18 || i === 19) {
                const target = document.createElement('div');
                target.className = 'scroll-target';
                target.textContent = `Alvo ${targetsClicked + 1} = Clique em mim!`;
                target.dataset.index = targetsClicked;
                
                target.addEventListener('click', (e) => {
                    e.target.style.backgroundColor = '#2ecc71';
                    e.target.textContent = 'Encontrado!';
                ...
                scrollContent.appendChild(target);
                targetsClicked = 0; // Reset for this level's counting
            }
        }
        
        scrollContainer.appendChild(scrollContent);
        gameDisplay.innerHTML = '';
        gameDisplay.appendChild(scrollContainer);
    }
